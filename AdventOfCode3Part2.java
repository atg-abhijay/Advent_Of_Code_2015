public class AdventOfCode3Part2 {
  public static void main(String[] args) {
    int[] santaDirection = {0, 0, 0, 0}; // north east west south for Santa
    int[] roboSantaDirection = {0, 0, 0, 0}; // north east west south for Robo Santa
    int x1 = 0;
    int y1 = 0;
    int x2 = 0;
    int y2 = 0;
    ...
    Coordinates[] housesVisitedBySanta = new Coordinates[input.length()/2];
    Coordinates[] housesVisitedByRoboSanta = new Coordinates[input.length()/2];
    int indiceForSanta = -1;
    int indiceForRoboSanta = -1;
    for(int i = 0; i < input.length(); i++) {
      if(i % 2 == 0) { // Santa delivering presents at houses with even indices - 0,2,4,6,8.....
        indiceForSanta++;
        if(input.charAt(i) == '^') {
          santaDirection[0]++;
          y1++;
        ...
        housesVisitedBySanta[indiceForSanta] = new Coordinates(x1, y1); 
        //System.out.println("Santa's stuff");
        //System.out.println((i+1) + ". " + housesVisitedBySanta[indiceForSanta].getX() + ", " + housesVisitedBySanta[indiceForSanta].getY());
        //System.out.println((i+1) + ". " + "(" + x + "," + y + ")");
      }
      
      
      else { // Robo Santa delivering presents at houses with odd indices - 1,3,5,7......
        indiceForRoboSanta++;
        if(input.charAt(i) == '^') {
          roboSantaDirection[0]++;
          y2++;
        }
        if(input.charAt(i) == '>') {
          roboSantaDirection[1]++;
          x2++;
        }
        if(input.charAt(i) == '<') {
          roboSantaDirection[2]++;
          x2--;
        }
        if(input.charAt(i) == 'v') {
          roboSantaDirection[3]++;
          y2--;
        }
        housesVisitedByRoboSanta[indiceForRoboSanta] = new Coordinates(x2, y2);
       // System.out.println("Robo Santa's stuff");
        //System.out.println((i+1) + ". " + housesVisitedByRoboSanta[indiceForRoboSanta].getX() + ", " + housesVisitedByRoboSanta[indiceForRoboSanta].getY());
      }
    }
    ...
    System.out.println();
    System.out.println("North " + santaDirection[0]);
    System.out.println("East " + santaDirection[1]);
    System.out.println("West " + santaDirection[2]);
    System.out.println("South " + santaDirection[3]);
    System.out.println("Total " + (santaDirection[0] + santaDirection[1] + santaDirection[2] + santaDirection[3]));
    ...
    System.out.println();
    System.out.println("North " + roboSantaDirection[0]);
    System.out.println("East " + roboSantaDirection[1]);
    System.out.println("West " + roboSantaDirection[2]);
    System.out.println("South " + roboSantaDirection[3]);
    System.out.println("Total " + (roboSantaDirection[0] + roboSantaDirection[1] + roboSantaDirection[2] + roboSantaDirection[3]));
    
    Coordinates[] housesVisited = new Coordinates[input.length()];
    for(int j = 0; j < input.length()/2; j++) {
      housesVisited[j] = housesVisitedBySanta[j];
      housesVisited[j + (input.length()/2)] = housesVisitedByRoboSanta[j];
    }
    double uniqueHouses = 0;
    for(int k = 0; k < housesVisited.length; k++) {
      uniqueHouses += housesVisited[k].numTimesCoord(housesVisited);
    }
    
    /* double resultFromSanta = 0;
    double resultFromRoboSanta = 0;
    double resultFromBoth = 0;
    for(int j = 0; j < housesVisitedBySanta.length; j++) {
      resultFromSanta += (1/(housesVisitedBySanta[j].numTimesCoord(housesVisitedBySanta)));
      resultFromRoboSanta += (1/(housesVisitedByRoboSanta[j].numTimesCoord(housesVisitedByRoboSanta)));
      resultFromBoth += 1/resultFromSanta + 1/resultFromRoboSanta;
    } */
    System.out.println(uniqueHouses);
